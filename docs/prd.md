...
| FIT-18 | A FIT file carrying no samples is resolved by what it says about itself, not treated as one kind of failure: an activity whose data was lost is recorded as having happened (excluded from totals, reviews and compliance rather than counted as zero), an activity that never ran is not recorded at all, and a file that is not an activity is neither. None is re-read once judged. Added 3 August 2026: two files on the deployment warned on every poll pass for three weeks, and they turned out to be Zwift abandoned starts rather than lost rides, so recording them would have claimed training on days already correctly credited. The lost-ride rule was settled in migration 015 and implemented only for upstream placeholders. | A file whose session names a start and a duration produces a session flagged `data_unavailable`, closes no prescription, is reviewed by nobody, and does not make its day missed; a file whose session records no elapsed time produces no row at all; a second scan pass logs nothing and the count appears in `poll`'s result. |
...
The coach is given permissions, not numbers. That is stated in `prompts/persona.md` and in section 2, and until P16 nothing enforced it: a prompt is a soft constraint, and a model trained on sports content has strong priors about what an FTP looks like.

| ID | Requirement | Acceptance |
| --- | --- | --- |
| TRUST-01 | The figures the model may state are those it was given: the assembled prompt, the tool results of the same turn, and the athlete's own messages. No provenance refactor of computed values is required to establish this. | The attribution for a turn is derivable from the prompt and the tool results alone. |
| TRUST-02 | Attribution has three structurally separate channels: tool and prompt figures, figures the athlete supplied himself, and small whole numbers that are ordinary prose. They are snapshotted before the model loop and never merged. | A self-reported LTHR passes; the same figure never appears in the grounded channel; a retry cannot add to either channel. |
| TRUST-03 | Physiological figures in a reply are matched in both directions, number-then-unit and unit-then-number, and checked against the attribution. Numbers inside string leaves of a tool result are not attribution. | A figure quoted from a tool passes; the same figure present only inside a timestamp string does not. |
| TRUST-04 | There is one implementation of what counts as an attributed figure. The weekly review's stricter policy — every number, not only physiological ones — is expressed over it rather than beside it. | Rewording either surface cannot change the other's matching rule. |
| TRUST-05 | The model has a named way to report a figure it does not have, which records the gap and returns a fixed user-safe sentence. The internal reason is never shown to the athlete. | Calling it stores the gap and returns a sentence containing no figure and no methodology. |
| TRUST-06 | Arguments the server can derive are stripped from the model's tool call unconditionally, not validated against the schema. | An undeclared `user_id` is discarded; a value passed in cannot return as attribution. |
| TRUST-07 | The scanner records without blocking until enforcement is enabled. Under enforcement a violation is retried once naming the figures and the way out, and a second failure substitutes a fallback rather than going silent. | Shadow mode sends the reply and records the hit. Enforcement retries, then substitutes. Neither path is ever silence. |
| TRUST-08 | A labelled corpus of violations, qualitative near misses and attributed figures is asserted in aggregate for zero false positives and zero false negatives, and is seeded from real transcripts. | The corpus is non-empty and both aggregate assertions hold. |
...
| P16 | Enforce the trust model at runtime rather than asking for it in the prompt. | TRUST-01 to TRUST-08 | The corpus holds at zero false positives and zero false negatives; a figure from a tool passes and the same figure from nowhere does not; the escape hatch records a gap and says nothing numeric; shadow mode records without blocking. | Ships in shadow. Enforcement is a separate decision, taken once the corpus has been reseeded from real transcripts read back through `coach-transcript` and holds clean. `docs/prior-art.md` section 1 is explicit that the corpus is what made rewriting this scanner three times safe, which is why P15 came first. |
...
| 12 | **Resolved on 5 August 2026. The Telegram bot token leaked, was scraped, and was used.** It was committed on 2 August in #20 as the fixture of `test_the_bot_token_never_reaches_a_log_line` — a test whose entire subject is keeping that token out of a log line — and this repository is public. Three days later spam arrived in the athlete's chat, sent *as the coach*, with an inline keyboard. That is not something a stranger can do to a one to one bot chat: it requires the token, so the arrival of the message was itself the proof of compromise. That token is burned: revoke it through BotFather (setup step 4) and never reinstate it. The fixture here is replaced with a declared fake. **Not** rewritten out of history: the repository is public and was cloned by at least one party who used what they found, so a force push would buy nothing real and cost every commit hash. Rotation is the remedy; the dead token in the log is harmless once it is dead. | — | The Anthropic key was never committed — checked against every blob in history, not just the working tree — and no other credential was either. SEC-01 now has the test its acceptance criterion always described, `test_no_real_credential_is_committed_to_this_repository`, which fails on the exact string that leaked. Rotation procedure in setup step 4. |
