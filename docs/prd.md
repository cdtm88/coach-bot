...
| SEC-04 | No OAuth flow exists anywhere in the system. All upstream access is by API key or secret URL held in the environment. **Under review:** intervals.icu attaches webhooks to OAuth applications, so FIT-01 may require a one time consent and a stored bearer token. Open item 11 decides whether this requirement is amended or FIT-01 is. | Codebase contains no OAuth client or token refresh logic. |
...
| 11 | **Webhooks require an OAuth application, and SEC-04 forbids OAuth.** Apps are created by emailing intervals.icu with a website and privacy policy URL, and webhooks are configured on the app's management page. The API key path explicitly excludes all of it. Unknown, and decisive: whether a webhook fires for the owner's own activities without an OAuth consent. If it does not, FIT-01 needs a one time consent and a stored bearer token, which SEC-04 was written to prevent. The alternatives are dropping to reconcile-only ingest, which breaks FIT-01's "without polling" and PERF-03's 5 minute budget, or amending SEC-04. **Blocks P03** and decides whether ingest is push or pull. | P03 | See `docs/intervals-api.md`. |
