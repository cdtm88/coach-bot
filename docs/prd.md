...
| P05 | Read recovery from intervals.icu wellness. | RECOV-01 to RECOV-06 | Every wellness field the feed carries lands each morning with no OAuth client anywhere in the codebase, and a withheld field degrades the deviation without failing ingest. | n/a. P04 landed RECOV-01, RECOV-02 and RECOV-05 as a side effect — body mass and recovery are the same read, and storing half the fields to come back for the rest would have cost a second pass and answered nothing. P05 is therefore RECOV-04 (the local deviation) and RECOV-06 (disambiguating a missing session) only. |
...
| # | Item | Blocks | Note |
| --- | --- | --- | --- |
| 1 | **Resolved. Where is the weight in intervals.icu coming from?** Nowhere. A 21 day wellness read on 28 July 2026 returned 22 rows, 13 of them populated, and `weight` null on every one. The question assumed it either moved or repeated; it does neither. Not the harmful case — there is no stale number for the coach to anchor on — but not the hoped-for case either. | — | Verified 28 July 2026, `docs/intervals-api.md` V3a. |
