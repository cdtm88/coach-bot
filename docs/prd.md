...
| 10 | **Does intervals.icu offer an upload webhook at all?** FIT-01 assumes one and the OpenAPI spec contains no `webhook`, `callback` or subscription path. Registration may live in the settings UI; that needs checking by someone logged in. If none exists, the 6 hour reconcile becomes the only ingest path, a ride waits up to 6 hours for its review, and PERF-03's 5 minute budget is unreachable without a shorter poll interval that FIT-01's own wording rules out. | P03 | Blocks FIT-01 and PERF-03 as written. See `docs/intervals-api.md`. |
