...
| 11 | **Register the app and capture its webhook secret.** Needed before P03: create the app at intervals.icu `/settings`, point the webhook at the tunnel route, and put the secret in `.env` as `INTERVALS_WEBHOOK_SECRET`. Confirm this is UI configuration only and does not force a runtime OAuth token exchange, which would conflict with SEC-04. | P03 | The `.env` key already exists in the template. |
