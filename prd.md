...
PRD v2. Supersedes the July 2026 PRD.
...
### Which design section governs which requirements

| Design section | Governs | Domains |
| --- | --- | --- |
| 2, 3 | Three tier model, per turn context assembly | MEM, CHAT |
| 4 | Full DDL for every table | MEM, CONS, PLAN, ADJ, OBS |
...
| 10 | Feeds, planning writes, adjustment authority | FIT, HLTH, RECOV, CALR, PLAN, ADJ, LOG |
...
| MEM-03 | Changes supersede rather than update: old row closed, new row written, pointer set. | After three changes to one key, the store returns one active row and two superseded rows in order. |
...
| MEM-06 | Every fact change writes a fact_events row with action, reason, actor and evidence. | Count of fact_events is greater than or equal to count of facts at all times. |
...
| MEM-08 | Derived rollups (7 and 28 day load, weight trend slope, adherence, recovery deviation) are computed in SQL, not by the model. | The rollup table is populated by a SQL job and the agent never receives raw session rows for arithmetic. |
| MEM-09 | Working memory is a single row of conversation state, rewritten per turn and cleared nightly. | After consolidation, today_uncommitted is empty. |
| MEM-10 | Standing memory is loaded in full on every turn; only episodic notes are retrieved on demand. | Assembled context contains all active facts and no vector search occurs. |
| MEM-11 | Per turn assembled context stays under 4,000 tokens excluding conversation history. | Measured token count logged per turn; p95 under 4,000. |
...
| ID | Requirement | Acceptance |
| --- | --- | --- |
| CONS-01 | A nightly job at 03:00 reads the day's messages, telemetry deltas, pending writes and active facts. | Job logs a run row with counts for each input. |
...
| CONS-06 | In-turn writes land in pending_writes and are only ratified by consolidation. | No path exists from a chat turn to a direct facts insert. |
| CONS-07 | Unconfirmed facts lose confidence by category half life; confidence never reaches zero and facts never silently vanish. | After simulated 90 days without confirmation, an availability fact is present with reduced confidence. |
| CONS-08 | Low confidence facts are flagged in context as candidates for natural verification, one per conversation maximum. | Context contains at most one verification candidate regardless of how many qualify. |
...
| CHAT-02 | Persona is loaded from a versioned system prompt file, seeded from the July 2026 coaching conversation. | Changing the file changes behaviour without a code deploy. |
| CHAT-03 | The agent never narrates memory operations in ordinary conversation. | No response contains a statement that something has been saved, noted or remembered. |
| CHAT-04 | The agent asks at most one question per message. | No outbound message contains two question marks in separate sentences. |
...
| SAFE-02 | Consolidation cannot create, change or supersede a safety key; only an explicit athlete statement can. | A seeded attempt is rejected and written to fact_events with actor and reason. |
...
| ID | Requirement | Acceptance |
| --- | --- | --- |
| FIT-01 | Activity ingest is triggered by an intervals.icu webhook on upload, with a scheduled reconcile every 6 hours as backstop. | A new Zwift ride appears as a session row within 5 minutes without polling. |
...
| FIT-06 | A session review is generated covering compliance and one forward looking coaching note. | Review written as an observation note within 5 minutes of ingest. |
...
| HLTH-15 | If no reading has arrived for more than 12 days, the coach mentions it once, lightly, and not again until a reading resets the counter. This is the only weigh in prompt in the system. Activity feed staleness is handled separately. | A gap of any length produces exactly one mention. |
...
| RECOV-02 | Sleep, resting heart rate, HRV, recovery score, respiration and SpO2 are stored per day from the wellness feed. | All six present for a day with complete Whoop data. |
...
| PLAN-04 | Sessions are never planned into busy time seen on the calendar feeds. | Seeded conflict causes a move or shortening, not an overlap. |
...
| PLAN-09 | Structured sessions publish as machine readable workout steps with duration and power targets, not prose descriptions. | A published session downloads as a valid zwo file with the intended intervals. |
...
| BLOCK-07 | Weekly planned load never increases by more than a configured percentage against the prior week. | Generation is rejected if the limit is breached. |
...
| ADJ-02 | Automatic changes may only reduce load: shorten, ease, move later, or convert to rest. | Any generated change that increases load is rejected and logged. |
...
| NOTIF-02 | If a session was prescribed and nothing has been uploaded by 21:00 local, the coach follows up once. | Follow-up fires once, not repeatedly, and not when an activity has already landed. |
...
| ID | Requirement | Acceptance |
| --- | --- | --- |
| BREAK-01 | Scheduled breaks can be set for holidays, travel or illness, with a start and end date. | A break can be created conversationally and is stored. |
...
| GYM-04 | Gym load is tracked as session count and RPE rather than tonnage. | Rollups contain gym session count and mean RPE per week. |
| GYM-05 | Gym sessions count toward the weekly load ceiling alongside cycling. | A week at the cycling ceiling cannot add a gym session without reducing elsewhere. |
...
| OBS-04 | Failures alert to the chat; successes are silent. | A passing night produces no message. |
| OBS-05 | Each feed records last success and surfaces staleness. | Feeds table accurate for all five feeds. |
| OBS-06 | Nightly backup of the database plus the markdown fact export. | Both present and restorable in a test restore. |
| OBS-07 | A daily spend cap enforces a hard stop, independent of any monthly alert. | Simulated runaway usage halts model calls the same day and notifies. |
...
| --- | --- | --- | --- |
| P00 | Stand up the memory store and its invariants so that facts can be written, superseded and audited. Implement directly against docs/memory-design.md sections 4 to 7. | MEM-01 to MEM-12, SPEC-01 to SPEC-04, SEC-01, SEC-04, SEC-05 | Schema migrated, seeded contradiction rejected by the index, audit trail returns full history for a key. |
| P01 | Make the coach converse naturally over Telegram with full standing memory in context. | CHAT-01 to CHAT-10, SAFE-01, SAFE-05, TZ-01 to TZ-03, MODEL-01 to MODEL-03 | A week of real conversation with no narrated memory operations and constraints present in every prompt. |
| P02 | Make memory self correcting through nightly consolidation. | CONS-01 to CONS-10, SAFE-02, SAFE-03 | Seeded contradictions resolve per the matrix, re-running a night creates nothing new, decay behaves. |

### M2 Sensing

| Phase | Goal | Requirements | Done when |
| --- | --- | --- | --- |
| P03 | Ingest activities from intervals.icu and produce session reviews against prescriptions. | FIT-01 to FIT-16 | Full history backfills silently, new rides review within five minutes, replayed webhooks ignored, reconcile restores a deleted session. |
| P04 | Accept macros from MacroLog and read body mass from wellness, with no third party export app. | HLTH-01 to HLTH-16 | Per-meal macros land from MacroLog and readings land at an irregular rate and the fitted trend refuses to make a directional claim until three points exist. |
| P05 | Read recovery from intervals.icu wellness. | RECOV-01 to RECOV-06 | All six wellness fields land each morning with no OAuth client anywhere in the codebase. |
| P06 | Read calendar feeds so the coach knows the shape of the week including golf and commitments. | CALR-01 to CALR-06 | Observed availability facts appear after two weeks of real calendar data. |

### M3 Coaching

| Phase | Goal | Requirements | Done when |
| --- | --- | --- | --- |
| P07 | Generate and maintain four week blocks with concrete cycling and gym prescriptions. | BLOCK-01 to BLOCK-08, GYM-01 to GYM-07, SAFE-04 | A full block generates across both disciplines, constraint violating movements are blocked, and the ramp test supersedes inferred physiology. |
| P08 | Publish prescriptions to intervals.icu and detect athlete edits. | PLAN-01 to PLAN-11 | Sessions appear as planned workouts, structured ones load in Zwift without file handling, edits round trip into observed availability, no duplicates after ten changes. |
| P09 | Let session data reshape the remaining week within bounded authority. | ADJ-01 to ADJ-08 | Downgrades fire automatically, an attempted upgrade is rejected, missing files never restructure alone. |
| P10 | Run the weekly review and the daily rhythm that close the loop. | REV-01 to REV-05, NOTIF-01 to NOTIF-06, BREAK-01 to BREAK-04, NUT-01 to NUT-06, LOG-01 to LOG-08 | Four consecutive Sunday reviews with figures and block updates, daily nudges firing correctly, and a simulated break suppressing them. |

### M4 Trust and polish

| Phase | Goal | Requirements | Done when |
| --- | --- | --- | --- |
| P11 | Accept voice notes as a first class input. | VOICE-01 to VOICE-03 | Post ride voice note produces a session discussion without typing. |
| P12 | Prove the system remembers correctly and know what it costs. | OBS-01 to OBS-09, PERF-01 to PERF-03 | Recall suite passes 30 of 30 for fourteen consecutive nights and daily cost is visible. |

## 5. Open items to resolve before building

1. **Where is the weight in intervals.icu coming from?** Read wellness across the last two weeks. If the value is identical on every date it is a static profile field copied from an upstream source, which is actively harmful because the coach would anchor on a stale number and never notice. HealthBridge then proceeds as specified. If it moves day to day, something already feeds it correctly and HealthBridge can be dropped.
2. **Verify no activity gaps after the Strava disconnect.** Compare the current activity list against the pre-disconnect snapshot and backfill anything missing from the local FIT archive through the upload endpoint.
3. **Confirm the manual activity endpoint** against the live swagger spec before committing to it in a phase, per LOG-07.
4. Transcription: hosted API or local model on the homelab.
5. Monthly cost ceiling to enforce.


176 requirements across 23 domains.
